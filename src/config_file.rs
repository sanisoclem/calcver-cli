...
impl CalcverFileConfig {
     pub fn convert(self) -> config::CalcverConfig {
        let project_defaults = config::VersionConfig::from_defaults();
        ...
        if let Some(c) =  self.cargo {
         ...
         config::CalcverConfig {
            project: config::VersionConfig {
...
pub fn from_config(config_path: &str) -> config::CalcverConfig {
...
fn parse_config(config_yaml: &str) -> config::CalcverConfig {
